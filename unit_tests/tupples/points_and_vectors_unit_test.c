...
/*   Updated: 2022/10/05 17:07:57 by sguilher         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	points_and_vectors_tests()
{
	...
	set_point(&point, 3, -2, 5);
	set_vector(&vector, -2, 3, 1);
	set_point(&expected, 1, 1, 6);
	add_point_and_vector_test(vector, point, expected);
	set_point(&expected, 5, -5, 4);
	subtract_point_and_vector_test(vector, point, expected);
	set_point(&point, 3, 2, 1);
	set_vector(&vector, 5, 6, 7);
	set_point(&expected, 8, 8, 8);
	add_point_and_vector_test(vector, point, expected);
	set_point(&expected, -2, -4, -6);
	subtract_point_and_vector_test(vector, point, expected);
}
