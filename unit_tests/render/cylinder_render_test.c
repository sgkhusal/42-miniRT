...
/*   Updated: 2022/11/09 14:11:19 by sguilher         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vector	set_pixel_color(t_point ray_origin, int x, int y, t_rt *rt)
{
	...
	xs = cylinder_intersection(ray, *(t_cylinder *)(rt->world.objects->obj));
	if (xs.count == 2)
		add_intersections(xs, rt->world.objects, &list);
	if (list.head)
		hit = get_hit_intersection(list);
	if (hit)
		return (get_cylinder_color(ray,
			(t_cylinder *)(rt->world.objects->obj), rt->world.light, hit));
